...
func main() {
	...
	if *stateNamesOutPath != "" {
		stateOutput := renderStateNames(java, sha256Hex(sourceJava))
		stateFormatted, err := format.Source([]byte(stateOutput))
		if err != nil {
			panic(fmt.Errorf("format generated Java state names: %w", err))
		}
		if err := os.WriteFile(*stateNamesOutPath, stateFormatted, 0o644); err != nil {
			panic(err)
		}
	}
...
func renderStateNames(states []javaState, javaHash string) string {
	var builder strings.Builder
	builder.WriteString("// Code generated by cmd/registrygen; DO NOT EDIT.\n")
	fmt.Fprintf(&builder, "// Java state source SHA-256: %s\n\n", javaHash)
	builder.WriteString("package data\n\n")
	fmt.Fprintf(&builder, "const Java1214BlockStateNameCount = %d\n\n", len(states))
	builder.WriteString("var Java1214BlockStateNames = [...]string{\n")
	for i, state := range states {
		if i%4 == 0 {
			builder.WriteString("\t")
		}
		fmt.Fprintf(&builder, "%q, ", state.key)
		if i%4 == 3 {
			builder.WriteByte('\n')
		}
	}
